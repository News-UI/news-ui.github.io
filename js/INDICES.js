...
.then(function(data){
    ...
    for(x in data["News"]["INDICES"]){

        let count = data["News"]["INDICES"][x]["no_of_news"];
        let company = data["News"]["INDICES"][x]["Index Name"];


        let graph_data_1 ={};

        var date_data = data["Plots"]["INDICES"][company]["date"].split(",");
 var close_data = data["Plots"]["INDICES"][company]["close"].split(",").map(Number);
 var open_data = data["Plots"]["INDICES"][company]["open"].split(",").map(Number);
 var low_data = data["Plots"]["INDICES"][company]["low"].split(",").map(Number);
 var high_data = data["Plots"]["INDICES"][company]["high"].split(",").map(Number);
 var volume_data = data["Plots"]["INDICES"][company]["volume"].split(",").map(Number);
...
title.innerHTML = "Index name: "+company;
...
let title_text = data["News"]["INDICES"][x]["News "+i.toString()]["title"];
let date_text = data["News"]["INDICES"][x]["News "+i.toString()]["publish_date"];
let summary_text = data["News"]["INDICES"][x]["News "+i.toString()]["text"];
let url_text = data["News"]["INDICES"][x]["News "+i.toString()]["url"];
